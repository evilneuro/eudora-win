...
fast(register struct match *m, char *start, char *stop, sopno startst, sopno stopst)
{
	...
	register int c = (start == m->beginp) ? OUT_OF_CHARSET : *(start-1);
	...
	for (;;) {
		...
		i = 0;
		if ( (lastc == '\n' && m->g->cflags&REG_NEWLINE) ||
				(lastc == OUT_OF_CHARSET && !(m->eflags&REG_NOTBOL)) ) {
			flagch = BOL;
			i = m->g->nbol;
		}
		if ( (c == '\n' && m->g->cflags&REG_NEWLINE) ||
				(c == OUT_OF_CHARSET && !(m->eflags&REG_NOTEOL)) ) {
		...
		if ( (flagch == BOL || (lastc != OUT_OF_CHARSET && !ISWORD(lastc))) &&
					(c != OUT_OF_CHARSET && ISWORD(c)) ) {
			flagch = BOW;
		}
		if ( (lastc != OUT_OF_CHARSET && ISWORD(lastc)) &&
				(flagch == EOL || (c != OUT_OF_CHARSET && !ISWORD(c))) ) {
			flagch = EOW;
		}
		...
		if (ISSET(st, stopst) || p == stop)
			break;		/* NOTE BREAK OUT_OF_CHARSET */

		/* no, we must deal with this character */
		ASSIGN(tmp, st);
		ASSIGN(st, fresh);
		assert(c != OUT_OF_CHARSET);
