...
#pragma warning (disable: 4018 4244 4100)
...
#	define	GOODFLAGS(f)	((f)&~REG_DUMP)
	...
	if (cflags&REG_EXTENDED)
		p_ere(p, OUT_OF_CHARSET);
	else if (cflags&REG_NOSPEC)
		p_str(p);
	else
		p_bre(p, OUT_OF_CHARSET, OUT_OF_CHARSET);
	EMIT(OEND, 0);
	g->laststate = THERE();
 ...
 */
static void
ordinary(register struct parse *p, int ch)
{
...
#pragma warning (default: 4018 4244 4100)