...
GetFullyQualifiedDSBase(char *fqBuf, int bufSize)
{
	...
	if (::GetModuleFileName(AfxGetInstanceHandle(), fqBuf, bufSize)) {
		...
		csText.LoadString(IDS_DIRSERV_NOMODNAME);
        csTitle.LoadString(IDS_DIRSERV_ERROR);
	    ::MessageBox(NULL, csText,
			 csTitle, MB_OK | MB_ICONEXCLAMATION);
	    retVal = false;
	}
    }
    else {
...
IsComponentAvailable(int comp)
{
	...
	CString csTitle;
	csTitle.LoadString(IDS_DIRSERV_ERROR);
	::MessageBox(NULL, buf, csTitle, MB_OK | MB_ICONEXCLAMATION);
	retVal = false;
    }
...
RegisterCOMObjects()
{
	...
	CString csTitle;
	csTitle.LoadString(IDS_DIRSERV_ERROR);
	::MessageBox(NULL, _ds_comp_pnf, csTitle, MB_OK | MB_ICONEXCLAMATION);
    }
...
bool
DoesTheFileExist(const char * const file)
{
    bool retVal      = false;
    char fqBuf[_MAX_PATH] = { '\0' };
