...
extern unsigned char DSSHORTCUT[];
...
DSLeftView::OnCreateClient()
{
	...
	UINT nStaticallyAssignedPanesX = minWidth;
	UINT nDynamicallyAssignedPanesX = rectClient.Width()/2;
	UINT nStaticallyAssignedTopPaneY = 55; 
	UINT nDynamicallyAssignedTopPaneY = rectClient.Height()/3; 

    // We need to get the position of the splitter bar.

    if (fRet) {
		
		topPaneY  = ::GetPrivateProfileInt(dsSection, "TopPaneY", nStaticallyAssignedTopPaneY,
										   INIPath);
		panesX = ::GetPrivateProfileInt(dsSection, "PanesX",
//                                      minWidth - (2 * xSpace), INIPath);
										nStaticallyAssignedPanesX, INIPath);
		...
		if (FALSE == GetIniShort(IDS_INI_USE_MY_DIRSERV_WINDOW_POSITION))
		{
			...
			panesX = __max(panesX,nStaticallyAssignedPanesX);
			// It's better to also compare with width of the client's rectangular area to have better size allocation
			panesX = __max(panesX,nDynamicallyAssignedPanesX);		

			// The max of these values
			topPaneY = __max(topPaneY,nStaticallyAssignedTopPaneY);
			// It's better to also compare with width of the client's rectangular area to have better size allocation
			topPaneY = __max(topPaneY,nDynamicallyAssignedTopPaneY);
...
DSLeftView::OnInitialUpdate()
{
	...
	CRect queryrect;
	m_ST_QueryLabel.GetWindowRect(queryrect);
	queryLabelWidth = queryrect.right - queryrect.left;

	CRect startstoprect;
	m_PB_QueryStartStop.GetWindowRect(startstoprect);
	pushButtonWidth        = startstoprect.right - startstoprect.left;

	CRect keepontoprect;
	m_CB_KeepOnTop.GetWindowRect(keepontoprect);
	keepOnTopWidth	= keepontoprect.right - keepontoprect.left;
	
    // Control metrics.
    // Horizontal and vertical space.
    xSpace                 = (XBaseUnit);
    ySpace                 = (YBaseUnit)      / 2;
    int halfYSpace        = (YBaseUnit)      / 4;
    ...
    CString csText;
	csText.LoadString(IDS_DIRSERV_STATUSREADY);
    m_ST_QueryResults.SetWindowText(csText);
...
DSLeftView::DisplayHits()
{
    if (theHits == 0) {
		CString csText;
		csText.LoadString(IDS_DIRSERV_STATUSCONNECTING);
        m_ST_QueryResults.SetWindowText(csText);
    }
    else {
		CString csText;
		csText.LoadString(IDS_DIRSERV_GETTINGDATAFND);
        char szHits[64];
        wsprintf(szHits, "%s %ld", csText.GetBuffer(100), theHits);
    ...
    for (int i = 0; i < nItemCount; i++) {
        ...
        if (GetResultsTocItemState(i, LVIS_SELECTED) & LVIS_SELECTED) {
            ...
            if (anyEmail == true) {
				...
				curName.TrimLeft("\" ");
				curName.TrimRight("\" ");
				if ((Comma = curName.Find(',')) >= 0)
				{
					CString NewName;

					NewName = curName.Right(curName.GetLength() - Comma - 1);
					NewName.TrimLeft();
					NewName.TrimRight();
					if (Comma)
					{
...
DSLeftView::ResetQueryState(_DS_QUERY_STATE _dsqst)
{
    ...
    case _STARTED:{
        ...
        DisplayHits();
		CString csStop;
		csStop.LoadString(IDS_DIRSERV_STOP);
        m_PB_QueryStartStop.SetWindowText(csStop);
        break;}
    case _STOPPED:{
		CString csStart;
		csStart.LoadString(IDS_DIRSERV_START);
        m_PB_QueryStartStop.SetWindowText(csStart);
        char szHits[128];
		CString csLabel;
		csLabel.LoadString(IDS_DIRSERV_STATRDYLSTFND);
        wsprintf(szHits, 
                 "%s %ld", csLabel.GetBuffer(100), theHits);
        m_ST_QueryResults.SetWindowText(szHits);
...
DSLeftView::StartQuery()
{
    ...
    if (pDS->pQuery) {
		CString csCancel;
		csCancel.LoadString(IDS_DIRSERV_CNCLCURQRY);
		CString csProgress;
		csProgress.LoadString(IDS_DIRSERV_QRYINPGRS);
        if (IDYES == MessageBox(csCancel,
                                csProgress,
    ...
    if (SUCCEEDED(hErr)) {
        char strQuery[512] = {'\0'};
		const bool bPhOnly = pDSRV->OnlyPhSelected();
		...
		if (bPhOnly)
		{
			LPCTSTR PhReturn = GetIniString(IDS_INI_PH_RETURN);
			if (PhReturn && *PhReturn)
			{
				if (*strQuery && strQuery[strlen(strQuery) - 1] != ' ' && *PhReturn != ' ')
					strcat(strQuery, " ");
				strcat(strQuery, PhReturn);
			}
		}

        hErr =  pDS->pQuery->Initialize(strQuery, bPhOnly? QUERY_FLG_RAWQUERY : QUERY_FLG_NONE,
                                        DS_NAME);

        if (FAILED(hErr)) {
			CString csTitle1, csTitle2;
			csTitle1.LoadString(IDS_DIRSERV_QUERYINITFAILED);
			csTitle2.LoadString(IDS_DIRSERV_QUERYFAILED);

            MessageBox(csTitle1, csTitle2,
                       MB_OK|MB_ICONERROR);
            pDS->pQuery->Release();
            pDS->pQuery = NULL;
            ResetQueryState(_STOPPED);
        ...
        if (SUCCEEDED(hErr)) {
			...
			CString csTitle1 ;
			csTitle1.LoadString(IDS_DIRSERV_COULDNOTADD2DB);
			CString csTitle2;
			csTitle2.LoadString(IDS_DIRSERV_QUERYFAILED);
            MessageBox(csTitle1, csTitle2,
                       MB_OK|MB_ICONERROR);
		...
		CString csText;
		csText.LoadString(IDS_DIRSERV_UN2CRQRYOBJ);
		CString csTitle;
		csTitle.LoadString(IDS_DIRSERV_QUERYFAILED);
        MessageBox(csText, csTitle,
                   MB_OK|MB_ICONERROR);
        ResetQueryState(_STOPPED);
    }   
}
...
void
DSLeftView::DoQuery(LPCTSTR QueryString)
{
	if (!QueryString || !*QueryString)
		GiveQueryInputFocus();
	else
	{
		m_EB_QueryInput.SetWindowText(QueryString);
		OnQueryStartStop();
	}
}
...
DSLeftView::ReflectQueryInTitle(const char *query)
{
    ...
    strcpy(titleBuf, csTitle.GetBuffer(56));
...
DSLeftView::InitializeExportToBuffer(void *pRecord)
{
    ...
    pDS->pDirServ->ExportToBuffer(pRec, ExportToBufferCB,
                                  (void *)(this));
}
...
{
	CString csText;
	csText.LoadString(IDS_STATUS_GETTINGDATA);
    m_ST_QueryResults.SetWindowText(csText);
    DSMainSplitterWnd *pDSMSW = static_cast<DSMainSplitterWnd *>(GetParent());
    DSRightView       *pDSRV  = static_cast<DSRightView *>
                                (pDSMSW->GetPane(0, 1));
...
{
	CString csText;
	csText.LoadString(IDS_DIRSERV_ERROR);
    MessageBox(s, csText, MB_OK);
}
...
void DSLeftView::Error( QUERY_STATUS nQueryStatus )
{
    ...
    case QUERY_ECONNECT:
		csText.LoadString(IDS_DIRSERV_QUERY_ECONNECT);
        wsprintf( szBuf,
                  csText.GetBuffer(255));
        break;

    case QUERY_EADDRESS:
		csText.LoadString(IDS_DIRSERV_QUERY_EADDRESS);
        wsprintf( szBuf,
                  csText.GetBuffer(255));
        break;
    ...
    case QUERY_EREAD:
    case QUERY_EWRITE:
		csText.LoadString(IDS_DIRSERV_QUERY_EWRITE);
        wsprintf( szBuf,
                  csText.GetBuffer(255));
        break;

    case QUERY_EOUTOFOMEMORY:
		csText.LoadString(IDS_DIRSERV_QUERY_EOUTOFOMEMORY);
        wsprintf( szBuf,
                  csText.GetBuffer(255));
        break;

    case QUERY_TIMEDOUT:
		csText.LoadString(IDS_DIRSERV_QUERY_TIMEDOUT);
        wsprintf( szBuf,
                  csText.GetBuffer(255) );
        break;

    case QUERY_INVALID_CREDENTIALS:
		csText.LoadString(IDS_DIRSERV_INVALID_CREDENTIALS);
        wsprintf( szBuf,
                  csText.GetBuffer(255));
        break;

    case QUERY_UNWILLING_TO_PERFORM:
		csText.LoadString(IDS_DIRSERV_UNWILLING_TO_PERFORM);
        wsprintf( szBuf,
                  csText.GetBuffer(255));
        break;

    case QUERY_INVALID_DN_SYNTAX:
		csText.LoadString(IDS_DIRSERV_INVALID_DN_SYNTAX);
        wsprintf( szBuf,
                  csText.GetBuffer(255));
        break;

    case QUERY_SERVER_DOWN:
		csText.LoadString(IDS_DIRSERV_SERVER_DOWN);
        wsprintf( szBuf,
                  csText.GetBuffer(255));
        break;

    default:
		csText.LoadString(IDS_DIRSERV_QUERYFAILED);
        wsprintf( szBuf, csText.GetBuffer(255) );
        break;
    }
...
//      ErrorDialog( IDR_DIRECTORY_SERVICES, szBuf );
		...
		CString csTitle;
		csTitle.LoadString(IDS_DIRSERV_DIRSERV);
        MessageBox( szBuf, csTitle, MB_OK | MB_ICONEXCLAMATION );
		}
}
...
DSLeftView::ProcessShortcut(WPARAM wParam)
{
    ...
    if( sc == DSSHORTCUT[SC_LV_QUERYINPUT])
	{
        m_EB_QueryInput.SetFocus();
        retVal = true;
    }

    else if( sc == DSSHORTCUT[SC_LV_STARTSTOP])
	{
    ...
    else if( sc == DSSHORTCUT[SC_LV_TO])
	{
        if (m_PB_MailTo.IsWindowEnabled() == TRUE) {
            m_PB_MailTo.SetFocus();
    ...
    else if( sc == DSSHORTCUT[SC_LV_CC])
	{
        if (m_PB_MailCc.IsWindowEnabled() == TRUE) {
    ...
    else if (sc == DSSHORTCUT[SC_LV_BCC])
	{
        if (m_PB_MailBcc.IsWindowEnabled() == TRUE) {
    ...
    else if( sc == DSSHORTCUT[SC_LV_KEEPONTOP]) // We are always enabled!
    {
		m_CB_KeepOnTop.SetFocus();
...
bool  DSLeftView::InitShortcuts()
{
    DSSHORTCUT[SC_LV_QUERYINPUT] = 'q';
    DSSHORTCUT[SC_LV_STARTSTOP] = 's';
    DSSHORTCUT[SC_LV_TO] = 't';
    DSSHORTCUT[SC_LV_CC] = 'c';
    DSSHORTCUT[SC_LV_BCC] = 'b';
    DSSHORTCUT[SC_LV_KEEPONTOP] = 'k';

	unsigned char sc = 0;
	
	sc = GetDlgShortcutChar(this, IDD_NDS_ST_QUERY_LABEL);
	if( sc >0)
		DSSHORTCUT[SC_LV_QUERYINPUT] = sc;

	sc = GetDlgShortcutChar(this, IDD_NDS_PB_QUERY_STARTSTOP);
	if( sc >0)
		DSSHORTCUT[SC_LV_STARTSTOP] = sc;


	sc = GetDlgShortcutChar(this, IDD_NDS_PB_MAIL_TO);
	if( sc >0)
		DSSHORTCUT[SC_LV_TO] = sc;


	sc = GetDlgShortcutChar(this, IDD_NDS_PB_MAIL_CC);
	if( sc >0)
		DSSHORTCUT[SC_LV_CC] = sc;

	sc = GetDlgShortcutChar(this, IDD_NDS_PB_MAIL_BCC);
	if( sc >0)
		DSSHORTCUT[SC_LV_BCC] = sc;


	sc = GetDlgShortcutChar(this, IDD_NDS_CB_KEEPONTOP);
	if( sc >0)
		DSSHORTCUT[SC_LV_KEEPONTOP] = sc;
