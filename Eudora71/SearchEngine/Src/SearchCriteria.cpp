...
struct
{
...
} gCategoryTable[] = 
	{
		...
		CRITERIA_VERB_NUM_COMPARE_TYPE,		CRITERIA_VALUE_JUNKSCORE_TYPE,	// junk score
...
bool SearchCriteria::GetCurValueJunkScore(CriteriaValueJunkScore &val) const
{
	ASSERT(m_pValueJunkScore);

	if (!m_pValueJunkScore)
		return (false);

	val = (*m_pValueJunkScore);

	return (true);
}

CriteriaValueJunkScore SearchCriteria::GetValueJunkScore()	const
{
	ASSERT(m_pValueJunkScore);
	return (*m_pValueJunkScore);
}
...
bool SearchCriteria::SetCurValueJunkScore(const CriteriaValueJunkScore &val)
{
	ASSERT(m_pObj);
...
void SearchCriteria::PreProcessForMatching()
{
	if( m_pVerbTxtCmp != NULL)
	{
		...
		{
			if (*m_pVerbTxtCmp == CRITERIA_TEXT_COMPARE_CONTAINSWORD)
			{
				m_pValueText->TrimLeft();
				m_pValueText->TrimRight();
			}

			// All other searches including "contains whole word" should be case-insensitive
