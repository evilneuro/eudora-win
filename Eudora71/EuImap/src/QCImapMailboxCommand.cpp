...
BOOL				bAutoSync /* = FALSE */) : QCMailboxCommand( pDirector, szName, szPathname, theType, theStatus  )
{
...
void*				pData )
{
		...
		if( pTocDoc != NULL )
		{
			if (pTocDoc->m_pImapMailbox)
			{
				...
				BOOL bUpdateSucceeded = 
					SUCCEEDED (pTocDoc->m_pImapMailbox->DoManualResync(pTocDoc,
																	   TRUE /*bCheckMail*/,
																	   FALSE /*bDownloadedOnly*/,
																	   FALSE /*bInBackground*/,
																	   (theAction == CA_IMAP_RESYNC_TREE) /*bDoSubMailboxes*/) );
	...
	else if ( theAction == CA_DELETE_MESSAGE || theAction == CA_UNDELETE_MESSAGE )
	{
		...
		#if 0  // Only needed for reposting which doesn't work.
	...
	else if ((theAction == CA_JUNK) || (theAction == CA_NOT_JUNK))
	{
		// Must have a TOC.
		CTocDoc*			pTocDoc = GetToc(GetPathname(), GetName());
		if (!pTocDoc)
		{
			ASSERT(0);
			return;
		...
		// If data, it MUST be a summary.  (If no data, process selected summaries.)
		CSummary	*pSummary = NULL;
		if (pData)
		{
			if (!((CObject*)pData)->IsKindOf(RUNTIME_CLASS(CSummary)))
		...
		// If the server was busy, we may have to repost this operation (JOK)
		// If reposting ever works see handling of CA_DELETE_MESSAGE above
		// for what we should be doing here. - dwiggins

		// Junk or UnJunk the message(s).
		pTocDoc->ImapJunkMessages((theAction == CA_JUNK), pSummary);
		return;
	}
