...
extern long  IMAPGssapiAuthenticator (authchallenge_t challenger,authrespond_t responder,
...
{ 
	"GSSAPI",						// Name
	 IMAPGssapiAuthenticator,		// Client responder,
};
...
{
  return (MAILADDRESS *) memset (fs_get (sizeof (MAILADDRESS)),0,sizeof (MAILADDRESS));
}
...
void mail_free_body_data (BODY *body)
{
  ...
  if (body->sztype) fs_give((void **) &body->sztype);
...
long mail_parse_date (MESSAGECACHE *elt, char *s)
{
  ...
  if (*s) {			/* time specification present? */
	...
	if (isdigit((unsigned char)*s) && isdigit((unsigned char)s[1]) && isdigit((unsigned char)s[2]) && isdigit((unsigned char)s[3])) {
