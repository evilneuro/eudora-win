...
{
public:
	HeaderDesc(long estSize = 0L, bool bIgnoreDisc = FALSE);    
	~HeaderDesc();											

	void Init();
	int Read(LineReader* pLR, bool bWriteHeader = true);
	...
	bool m_bIgnoreDisc;// bIgnoreDisc is used in a special case	
					//just to create the CTLMime object from a buffer.
					//no disc operations will be performed.
};
