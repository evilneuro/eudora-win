...
public:
	...
	long m_importFlags;  // default is everything
...
private:
	...
	bool MakeTranslator( PaigeExportObject* pTxr, FormatType fmt = kFmtText, bool bIsForClipboard = false );
	bool MakeTranslator( PaigeImportObject* pTxr, FormatType fmt = kFmtText );

public:
	// must use this constructor
	PgDataTranslator( pg_ref pg, long flags = 0 );

	// Paige memory io
	void ExportData( memory_ref* expMemRef, FormatType fmt = kFmtText );
	void ImportData( memory_ref* impMemRef, FormatType fmt = kFmtText, short draw_mode = best_way );
...
{
public:
	CMyPaigeHTMLExportFilter(bool bIsForClipboard = false);
