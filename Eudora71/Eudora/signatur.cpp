...
void CSignatureView::OnUpdateEditPaste( CCmdUI* pCmdUI ) 
{
    if ( !m_fRO ) {
...
void CSignatureView::OnUpdateInsertPicture(CCmdUI* pCmdUI) 
{
    pCmdUI->Enable( FALSE );
}
...
void CSignatureView::Serialize(CArchive& ar)
{
	...
	else 
	{
		...
		int bytesToRead = static_cast<int>( sigFile->GetLength() );

		sigText = DEBUG_NEW char[bytesToRead + 1];
		...
		if ( IsFancy(sigText) == IS_RICH )
		{
			...
			char *NewSigText = DEBUG_NEW char[NewSize + 1];
