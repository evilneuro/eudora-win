...
QCMailboxCommand* QComFolder::GetMailboxCommand(void)
{
	...
	catch (CMemoryException * /* pMemoryException */)
	{
		...
		ASSERT( !"Rethrowing CMemoryException in QComFolder::GetMailboxCommand" );
		throw;
	}
	catch (CException * pException)
	{
		// Other MFC exception
		pException->Delete();
		ASSERT( !"Caught CException (not CMemoryException) in QComFolder::GetMailboxCommand" );
		success = false;
	}
	catch (std::bad_alloc & /* exception */)
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing std::bad_alloc in QComFolder::GetMailboxCommand" );
		throw;
	}
	catch (std::exception & /* exception */)
	{
		ASSERT( !"Caught std::exception (not std::bad_alloc) in QComFolder::GetMailboxCommand" );
		success = false;
	}
...
STDMETHODIMP QComFolder::Move(IEuFolder * NewParentFolder)
{
	...
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing CMemoryException in QComFolder::Move" );
		throw;
	}
	catch (CException * pException)
	{
		// Other MFC exception
		pException->Delete();
		ASSERT( !"Caught CException (not CMemoryException) in QComFolder::Move" );
		return E_FAIL;
	}
	catch (std::bad_alloc & /* exception */)
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing std::bad_alloc in QComFolder::Move" );
		throw;
	}
	catch (std::exception & /* exception */)
	{
		ASSERT( !"Caught std::exception (not std::bad_alloc) in QComFolder::Move" );
		return E_FAIL;
	}
...
STDMETHODIMP QComFolder::get_bContainsUnreadMessages(BOOL * pVal)
{
	...
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing CMemoryException in QComFolder::get_bContainsUnreadMessages" );
		throw;
	}
	catch (CException * pException)
	{
		// Other MFC exception
		pException->Delete();
		ASSERT( !"Caught CException (not CMemoryException) in QComFolder::get_bContainsUnreadMessages" );
		return E_FAIL;
	}
	catch (std::bad_alloc & /* exception */)
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing std::bad_alloc in QComFolder::get_bContainsUnreadMessages" );
		throw;
	}
	catch (std::exception & /* exception */)
	{
		ASSERT( !"Caught std::exception (not std::bad_alloc) in QComFolder::get_bContainsUnreadMessages" );
		return E_FAIL;
	}
...
STDMETHODIMP QComFolder::Resynchronize()
{
	CAutomationCall c;
	CTocDoc* pTocDoc = GetToc(m_FullName);
	if (pTocDoc && m_bIsImap)
	{
	QCMailboxCommand* pImapFolder = GetMailboxCommand();
	if( pImapFolder != NULL )
