						 ...
						 int& nWidth, int& nHeight, int nImgWidth)
{
	...
	int yImg = (nHeight - m_pToolBar->GetImgHeight())/2 + y + 1;
	...
	data.m_drawDC.SetBkColor(::GetSysColor(COLOR_WINDOW));

	//if message contains BP addresses, then draw a border around
	//the send button in the color code set for BP
	if(IsBPWarning()) {
		CBrush BPBrush(CBossProtector::GetBPColorCode()), *pBrush;
		pBrush = &BPBrush;
		CRect BorderRect(x, y, x+nWidth, y+nHeight);
		data.m_drawDC.FrameRect(LPCRECT(BorderRect), pBrush);
	}
	...
	//
	{
		...
		ASSERT(pTempFont);
		COLORREF rgbOldBgnd;
		rgbOldBgnd = data.m_drawDC.SetBkColor(secData.clrBtnFace);

		COLORREF rgbOldText;

		rgbOldText = data.m_drawDC.SetTextColor(::GetSysColor(COLOR_BTNTEXT));
...
//FORNOW			data.m_drawDC.TextOut(x + nImgWidth + 3, y + 3, strLabel);
			data.m_drawDC.DrawText(strLabel, CRect(x+nImgWidth, y+1, nWidth, nHeight), 
...
int TBarSendButton::OnToolHitTest(CPoint point, TOOLINFO * pTI) const
{
	if(pTI) {
		pTI->uFlags      = 0;
		pTI->hwnd        = m_pToolBar->m_hWnd;
		if(m_bBPWarning) 
			pTI->lpszText    = strdup(const_cast<char*>(GetIniString(IDS_BP_WARNING)));
		else 
