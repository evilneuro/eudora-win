...
// MFC did a dumb thing.  CControlBars have their own styles, but they
// don't provide a way to separate the regular windows WS_ styles from their
// own styles.  So you either can't use any regular windows WS_ styles, or
// if you do it winds up setting those flags for the specific CControlBar
// style.  If you do that, then MFC will ASSERT on you because the CControlBar
// styles contains non-CControlBar styles in it.
//
// So, to get around this we just strip the regular windows WS_ styles for the
// Create() call, and then add them after the object is created.
//
// JB 6/21/00

BOOL CWazooBar::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, UINT nID,
...
BOOL CWazooBar::Create(CWnd* pParentWnd, LPCTSTR lpszWindowName, DWORD dwStyle,
						   DWORD dwExStyle, UINT nID, CCreateContext* pContext)
{
	return CWazooBar::Create(NULL, lpszWindowName, nID, dwStyle,
		dwExStyle, CRect(0,0,0,0), pParentWnd, pContext);
}
...
//
////////////////////////////////////////////////////////////////////////
/*virtual*/ BOOL CWazooBar::ActivateWazooWindow(CRuntimeClass* pRuntimeClass)
{
...
CWazooWnd* CWazooBar::GetActiveWazooWindow()
{
	...
	if (pActiveTabWnd)
	{
		...
		if (pActiveTabWnd->IsWindowVisible() == FALSE)
			return NULL;
	}
...
{
	// Activate the last tab that was topmost when we last shut down
	ASSERT(m_wndTab.GetSafeHwnd());
	if (m_wndTab.GetTabCount() > 0)
	{
		m_wndTab.ActivateTab(m_LastActiveTabOnShutdown);
		m_wndTab.ScrollToTab(m_LastActiveTabOnShutdown);
	}
}
...
#ifdef INCLUDE_WEB_WAZOO
		CreateWazooWindow(RUNTIME_CLASS(CWebWazooWnd));
#endif
		break;
	case 1:
		...
		CreateWazooWindow(RUNTIME_CLASS(CLinkHistoryWazooWnd));
		break;
	case 2:
...
BOOL CWazooBar::LoadWazooConfigFromIni()
{
	...
	//
	// Sanity checking on some window sizes/locations
	//
	m_szDockHorz.cx = max(m_szDockHorz.cx, 20);
	m_szDockHorz.cy = max(m_szDockHorz.cy, 20);
	m_szDockVert.cx = max(m_szDockVert.cx, 20);
	m_szDockVert.cy = max(m_szDockVert.cy, 20);
	m_szFloat.cx = max(m_szFloat.cx, 20);
	m_szFloat.cy = max(m_szFloat.cy, 20);

	//
	// Create a key name by appending the control bar ID to a
	// "WazooBar" string as in "WazooBar201".
	//
	CString strName;
	...
	{
		if (isdigit((unsigned char)strParam[0]))
		{
			int nIndex = atoi(strParam);
			if (nIndex > 0)
				m_LastActiveTabOnShutdown = nIndex;
		}
		//
		...
		//
			...
			CreateWazooWindow(RUNTIME_CLASS(CTaskErrorWazooWnd));
		else if (strcmp(strParam, RUNTIME_CLASS(CLinkHistoryWazooWnd)->m_lpszClassName) == 0)
			CreateWazooWindow(RUNTIME_CLASS(CLinkHistoryWazooWnd));
#ifdef INCLUDE_WEB_WAZOO
		else if (strcmp(strParam, RUNTIME_CLASS(CWebWazooWnd)->m_lpszClassName) == 0)
			CreateWazooWindow(RUNTIME_CLASS(CWebWazooWnd));
#endif
		else
		{
	...
	if (!IsMDIChild())
	{
		if (m_wndTab.GetTabCount() <= 0)
		{
			// No wazoo windows in here.  How'd that happen?
			// Hide this guy so he doesn't cause any trouble.
			ASSERT(0);
			OnHide();
		}
		else if (IsFloating())
	...
	if (IsMDIChild())
	{
		...
		//
		if (m_wndTab.GetTabCount() > 0 && nFlags & FLAGS_VISIBLE)
		{
			switch (nCmdShow)
			{
...
BOOL CWazooBar::OnEraseBkgnd(CDC* pDC)
{
	...
	// Fill background with COLOR_BTNFACE
