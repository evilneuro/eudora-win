...
void QCAutomationDirector::InternalMoveFolder(QCMailboxCommand* pTravel, QCMailboxCommand* pTo)
{
	...
	catch (CMemoryException * /* pMemoryException */)
	{
		...
		ASSERT( !"Rethrowing CMemoryException in QCAutomationDirector::InternalMoveFolder" );
		throw;
	}
	catch (CException * pException)
	{
		// Other MFC exception
		pException->Delete();
		ASSERT( !"Caught CException (not CMemoryException) in QCAutomationDirector::InternalMoveFolder" );
	}
	catch (std::bad_alloc & /* exception */)
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing std::bad_alloc in QCAutomationDirector::InternalMoveFolder" );
		throw;
	}
	catch (std::exception & /* exception */)
	{
		ASSERT( !"Caught std::exception (not std::bad_alloc) in QCAutomationDirector::InternalMoveFolder" );
	}
}
...
void QCAutomationDirector::InternalRenameFolder(QCMailboxCommand* pRename)
{
	...
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing CMemoryException in QCAutomationDirector::InternalRenameFolder" );
		throw;
	}
	catch (CException * pException)
	{
		// Other MFC exception
		pException->Delete();
		ASSERT( !"Caught CException (not CMemoryException) in QCAutomationDirector::InternalRenameFolder" );
	}
	catch (std::bad_alloc & /* exception */)
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing std::bad_alloc in QCAutomationDirector::InternalRenameFolder" );
		throw;
	}
	catch (std::exception & /* exception */)
	{
		ASSERT( !"Caught std::exception (not std::bad_alloc) in QCAutomationDirector::InternalRenameFolder" );
	}
}
