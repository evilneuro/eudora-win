...
CStatusBarEx::CStatusBarEx()
{
	...
	m_TaskImageList = DEBUG_NEW QCImageList;
	// c_dalew 9/30/99 - Changed Create() to Load() now that we use QCImageList.
	VERIFY(m_TaskImageList->Load(nImageListWidth, nImageListWidth, 0, MAKEINTRESOURCE(nImageListBitmapID), nImageListBgColor));
...
// Override to get rid of non-CControlBar styles
//
...
void CStatusBarEx::OnSize(UINT nType, int cx, int cy) 
{
	...
	if (m_bInitToolTip)
	{
		...
		SetTooltipRect(NUM_LOCK_INDEX, NUM_LOCK_INDEX);
		SetTooltipRect(CAPS_LOCK_INDEX, CAPS_LOCK_INDEX);
		SetTooltipRect(OVR_LOCK_INDEX, OVR_LOCK_INDEX);
	}
}
...
bool CStatusBarEx::HandleRightClick(UINT nFlags, CPoint point)
{
	...
	switch (idx)
	{
		case TASK_STATUS_INDEX:
		{
			...
			int		nCheckedFlag = bOffline ? MF_CHECKED : MF_UNCHECKED;
			menu.AppendMenu(MF_STRING | nCheckedFlag, nMENUID_OfflineToggle, "Work Offline");
