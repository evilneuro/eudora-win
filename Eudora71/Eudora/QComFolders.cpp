...
bool QComFolders::InsertAtEnd(QComFolder* pNewFolder)
{
	...
	catch (CMemoryException * /* pMemoryException */)
	{
		...
		ASSERT( !"Rethrowing CMemoryException in QComFolders::InsertAtEnd" );
		throw;
	}
	catch (CException * pException)
	{
		// Other MFC exception
		pException->Delete();
		ASSERT( !"Caught CException (not CMemoryException) in QComFolders::InsertAtEnd" );
		rc = false;
	}
	catch (std::bad_alloc & /* exception */)
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing std::bad_alloc in QComFolders::InsertAtEnd" );
		throw;
	}
	catch (std::exception & /* exception */)
	{
		ASSERT( !"Caught std::exception (not std::bad_alloc) in QComFolders::InsertAtEnd" );
		rc = false;
	}
...
bool QComFolders::InsertAtIndex(QComFolder* pNewFolder, long Index)
{
	...
	while (pos)
	{
		...
		if (pFolder->m_Index >= Index)
		{
			...
			{
				// Catastrophic memory exception - rethrow
				ASSERT( !"Rethrowing CMemoryException in QComFolders::InsertAtIndex" );
				throw;
			}
			catch (CException * pException)
			{
				// Other MFC exception
				pException->Delete();
				ASSERT( !"Caught CException (not CMemoryException) in QComFolders::InsertAtIndex" );
				rc = false;
			}
			catch (std::bad_alloc & /* exception */)
			{
				// Catastrophic memory exception - rethrow
				ASSERT( !"Rethrowing std::bad_alloc in QComFolders::InsertAtIndex" );
				throw;
			}
			catch (std::exception & /* exception */)
			{
				ASSERT( !"Caught std::exception (not std::bad_alloc) in QComFolders::InsertAtIndex" );
				rc = false;
			}
...
STDMETHODIMP QComFolders::RemoveByID(long ID)
{
	...
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing CMemoryException in QComFolders::RemoveByID" );
		throw;
	}
	catch (CException * pException)
	{
		// Other MFC exception
		pException->Delete();
		ASSERT( !"Caught CException (not CMemoryException) in QComFolders::RemoveByID" );
		return E_FAIL;
	}
	catch (std::bad_alloc & /* exception */)
	{
		// Catastrophic memory exception - rethrow
		ASSERT( !"Rethrowing std::bad_alloc in QComFolders::RemoveByID" );
		throw;
	}
	catch (std::exception & /* exception */)
	{
		ASSERT( !"Caught std::exception (not std::bad_alloc) in QComFolders::RemoveByID" );
		return E_FAIL;
	}
