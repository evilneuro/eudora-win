...
void EncodeThree64(const char *In, char*& Out, int& bpl, bool bWrapLines)
{
	if (bWrapLines)
	{
		if (bpl==76)
...
{
	m_bWrapLines = (flag & kDontWrapLines) == 0;
...
int Base64Encoder::Done(char* Out, LONG& OutLen)
{
	...
	if (m_PartialCount)
	{
		if (m_PartialCount < 2)
			m_Partial[1] = 0;
		m_Partial[2] = 0;
		EncodeThree64(m_Partial, OutSpot, m_BytesOnLine, m_bWrapLines);
...
int Base64Encoder::Encode(const char* In, LONG InLen, char* Out, LONG& OutLen)
{
	...
	for (; InLen >= 3; InSpot += 3, InLen -= 3)
		EncodeThree64(InSpot, OutSpot, m_BytesOnLine, m_bWrapLines);
