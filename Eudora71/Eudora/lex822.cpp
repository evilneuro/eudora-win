...
Lex822State::Lex822State(LineReader* pLR, bool shouldOutputToFile, JJFile* pOutputFile)
	:	m_LR(pLR), m_ShouldOutputToFile(shouldOutputToFile), m_OutputFile(pOutputFile)
{
	State = Init822;
}
...
Token822Enum Lex822State::WriteHeaderToken(Token822Enum tokenType)
{
	...
	if (m_ShouldOutputToFile)
	{
		//	We write to m_OutputFile if someone specified it in the constructor
		//	or to g_pPOPInboxFile if not (indicated by m_OutputFile == NULL).
		JJFile*	pOutputFile = (m_OutputFile != NULL) ? m_OutputFile : g_pPOPInboxFile;
		
		switch (tokenType)
		{
			// Either of these get written as two returns
			case EndOfMessage:
				if (FAILED(pOutputFile->PutLine()) ||
					FAILED(pOutputFile->PutLine()))
				{
					err = -1;
				}
				break;
			...
			case QText:
				if (FAILED(pOutputFile->Put('"')) ||
					FAILED(pOutputFile->Put((char*)Token, TokenSpot)) ||
					FAILED(pOutputFile->Put('"')))
				{
					err = -1;
				}
				break;
			...
			if (Token[0] != '\r')
	#endif
				if (FAILED(pOutputFile->Put((char*)Token, TokenSpot)))
					err = -1;
				break;
		}
	}
...
// This was code to support a legacy mailer that was sending out a line with a
// space instead of a blank line to end the headers.  Since spammers are now
// using this trick to get spam-labelling headers to be ignored, it's more
// important that we ditch it.  beckley - 3/12/03
//
