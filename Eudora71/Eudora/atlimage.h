	...
	DWORD dwExclude, TCHAR chSeparator )
{
	...
	for( UINT iCodec = 0; iCodec < nCodecs; iCodec++ )
	{
		...
		{
			LPCTSTR pwszFilenameExtension = CW2CT( pCodec->FilenameExtension );
...
inline HRESULT CImage::CreateFromGdiplusBitmap( Gdiplus::Bitmap& bmSrc ) throw()
{
	...
	// QC change
	// Need to check for palette size greater than 0. Otherwise GetPalette does
	// nothing, the memory isn't initialized, and the loop continues for too
	// long (because pPalette->Count isn't initialized) and we get a buffer
	// overflow crash.
	//if( Gdiplus::IsIndexedPixelFormat( eSrcPixelFormat ) )
	if( Gdiplus::IsIndexedPixelFormat( eSrcPixelFormat ) &&
		(bmSrc.GetPaletteSize() > 0) )
	{
	...
	if( eDestPixelFormat == eSrcPixelFormat )
	{
		...
		// QC change
		// Uh, LockBits can fail. It would be nice to check the return value here.
		//bmSrc.LockBits( &rect, Gdiplus::ImageLockModeRead, eSrcPixelFormat, &data );
		Gdiplus::Status status = bmSrc.LockBits( &rect, Gdiplus::ImageLockModeRead, eSrcPixelFormat, &data );
		if( status != Gdiplus::Ok )
		{
			return( E_FAIL );
		}
