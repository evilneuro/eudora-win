...
//		return (NULL);
	...
	if (!NetConnection)
	{
		if (FileIO)
			...
			NetConnection = DEBUG_NEW_NOTHROW CFileIO;
		else
		{
retry:
			if (GetIniShort(IDS_INI_CONNECT_WINSOCK))
			{
				QCNetworkSettings *settings = DEBUG_NEW QCNetworkSettings(g_Personalities.GetCurrent());
				...
				NetConnection = DEBUG_NEW_NOTHROW QCWorkerSocket(settings);
			}
			else
			{
				//
...
//		return (NULL);
	...
	if (FileIO)
		theConnection = DEBUG_NEW_NOTHROW CFileIO;
	else
	{
		theConnection = DEBUG_NEW_NOTHROW QCWorkerSocket(pSettings);

#if 0		
retry:
		if (GetIniShort(IDS_INI_CONNECT_WINSOCK))
			theConnection = DEBUG_NEW_NOTHROW QCWorkerSocket;
