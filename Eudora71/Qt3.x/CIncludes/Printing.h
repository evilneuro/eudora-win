...
EXTERN_API( TPPrPortRef )
PrOpenDoc						(THPrint 				hPrint,
								 TPPrPortRef 			pPrPort,
								 Ptr 					pIOBuf)								FOURWORDINLINE(0x2F3C, 0x0400, 0x0C00, 0xA8FD);

EXTERN_API( void )
PrCloseDoc						(TPPrPortRef 			pPrPort)							FOURWORDINLINE(0x2F3C, 0x0800, 0x0484, 0xA8FD);

EXTERN_API( void )
PrOpenPage						(TPPrPortRef 			pPrPort,
								 TPRect 				pPageFrame)							FOURWORDINLINE(0x2F3C, 0x1000, 0x0808, 0xA8FD);

EXTERN_API( void )
PrClosePage						(TPPrPortRef 			pPrPort)							FOURWORDINLINE(0x2F3C, 0x1800, 0x040C, 0xA8FD);
...
PrPicFile						(THPrint 				hPrint,
								 ...
								 Ptr 					pIOBuf,
								 Ptr 					pDevBuf,
								 TPPrStatusRef 			prStatus)							FOURWORDINLINE(0x2F3C, 0x6005, 0x1480, 0xA8FD);
