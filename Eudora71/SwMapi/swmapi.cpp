...
BOOL CSwitchApp::InitInstance()
{
    ...
    if (m_lpCmdLine[0] != '\0')
    {
		...
		while (! cmdline.IsEmpty())
		{
			...
			if (arg.CompareNoCase("/install:all") == 0)
			{
				//
				// Do install.
				//
				BOOL bRet = TRUE;
				bRet = InstallEudoraAsNetscapeMailHandler(FALSE);
				bRet &= InstallMAPI();

				// if either of them fail, we fool AFX into quitting immediately
				if (bRet == FALSE)
					return FALSE;
				
			}
			else if (arg.CompareNoCase("/uninstall:all") == 0)
			{
				//
				// Do uninstall.
				//
				BOOL bRet = TRUE;
				bRet = UnInstallEudoraAsNetscapeMailHandler(FALSE);
				bRet &= UnInstallMAPI();
			...
			else if (arg.CompareNoCase("/install:MAPI") == 0)
			{
				//
				// Do install.
				//				
				if (InstallMAPI() == FALSE)
					return FALSE;
				
			}
			else if (arg.CompareNoCase("/uninstall:MAPI") == 0)
			{
				//
				// Do uninstall.
				//				
				if (UnInstallMAPI() == FALSE)
					return FALSE;			
				
			}
			else if (arg.CompareNoCase("/install:NSHandler") == 0)
			{
				//
				// Do install.
				//
				if (FALSE == InstallEudoraAsNetscapeMailHandler(FALSE))
					return FALSE;
				
			}
			else if (arg.CompareNoCase("/uninstall:NSHandler") == 0)
			{
				//
				// Do uninstall.
				//
				if(FALSE == UnInstallEudoraAsNetscapeMailHandler(FALSE))
					return FALSE;			
...
BOOL CSwitchApp::InstallEudoraAsNetscapeMailHandler(BOOL bShowMessageBoxes /* TRUE */)
{
	BOOL bRet = TRUE;

	if (TRUE == CheckIfNetscapePrefsFileUpdated())
	{
		if (bShowMessageBoxes == TRUE ) 
			AfxMessageBox(IDS_NETSCAPE_PREFS_FILE_UPDATED,MB_ICONINFORMATION | MB_OK);
	}
	else
	{
		if (TRUE == (bRet = UpdateNetscapePrefsFile(TRUE)))
		{
			if (bShowMessageBoxes == TRUE ) 
				AfxMessageBox(IDS_NETSCAPE_HANDLER_INSTALL_OK,MB_ICONINFORMATION | MB_OK);
		}
		else
		{
			if (bShowMessageBoxes == TRUE ) 
				AfxMessageBox(IDS_NETSCAPE_HANDLER_INSTALL_ERROR,MB_ICONINFORMATION | MB_OK);
		}
	}
	return bRet;
}
...
BOOL CSwitchApp::UnInstallEudoraAsNetscapeMailHandler(BOOL bShowMessageBoxes /* TRUE */)
{
	BOOL bRet = TRUE;

	if (TRUE == CheckIfNetscapePrefsFileUpdated())
	{	
		if (TRUE == (bRet = UpdateNetscapePrefsFile(FALSE)))
		{
			if (bShowMessageBoxes == TRUE ) 
				AfxMessageBox(IDS_NETSCAPE_HANDLER_UNINSTALL_OK,MB_ICONINFORMATION | MB_OK);
		}
		else
		{
			if (bShowMessageBoxes == TRUE ) 
				AfxMessageBox(IDS_NETSCAPE_HANDLER_UNINSTALL_ERROR,MB_ICONINFORMATION | MB_OK);
		}
	}
	else
	{
		if (bShowMessageBoxes == TRUE ) 
			AfxMessageBox(IDS_NETSCAPE_PREFS_FILE_NOT_UPDATED,MB_ICONINFORMATION | MB_OK);
	}
