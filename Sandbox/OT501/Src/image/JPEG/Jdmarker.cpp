...
#define MAKE_BYTE_AVAIL(cinfo,action)  \
	if (bytes_in_buffer == 0) {  \
	  if (! (this->*datasrc->fill_input_buffer) (cinfo))  \
	    { action; }  \
	  INPUT_RELOAD(cinfo);  \
	}  \
	bytes_in_buffer--
...
#define INPUT_BYTE(cinfo,V,action)  \
	MAKESTMT( MAKE_BYTE_AVAIL(cinfo,action); \
		  V = (BYTE)GETJOCTET(*next_input_byte++); )
...
// static boolean
boolean SECJpeg::get_soi (j_decompress_ptr cinfo)
...
// static boolean
boolean SECJpeg::get_sof (j_decompress_ptr cinfo, boolean is_prog, boolean is_arith)
/* Process a SOFn marker */
{
  ...
  if (cinfo->comp_info == NULL)	/* do only once, even if suspend */
    cinfo->comp_info = (jpeg_component_info *) (this->*cinfo->mem->alloc_small)
...
// static boolean
boolean SECJpeg::get_sos (j_decompress_ptr cinfo)
...
// METHODDEF boolean
boolean SECJpeg::get_app0 (j_decompress_ptr cinfo)
...
#define JFIF_LEN 14
  ...
  if (length > 0)		/* skip any remaining data -- could be lots */
    (this->*cinfo->src->skip_input_data) (cinfo, (long) length);
...
// METHODDEF boolean
boolean SECJpeg::get_app14 (j_decompress_ptr cinfo)
...
// static boolean
boolean SECJpeg::get_dac (j_decompress_ptr cinfo)
...
// static boolean
boolean SECJpeg::get_dht (j_decompress_ptr cinfo)
...
// static boolean
boolean SECJpeg::get_dqt (j_decompress_ptr cinfo)
...
// static boolean
boolean SECJpeg::get_dri (j_decompress_ptr cinfo)
...
// METHODDEF boolean
boolean SECJpeg::skip_variable (j_decompress_ptr cinfo)
/* Skip over an unknown or uninteresting variable-length marker */
{
  ...
  (this->*cinfo->src->skip_input_data) (cinfo, (long) length - 2L);
...
// static boolean
boolean SECJpeg::next_marker (j_decompress_ptr cinfo)
{
  int c;
  INPUT_VARS(cinfo);
...
// static boolean
boolean SECJpeg::first_marker (j_decompress_ptr cinfo)
...
// METHODDEF int
int SECJpeg::read_markers (j_decompress_ptr cinfo)
{
  ...
  for (;;) {
    ...
    case M_APP14:
    case M_APP15:
      if (! (this->*cinfo->marker->process_APPn[cinfo->unread_marker - (int) M_APP0]) (cinfo))
	return JPEG_SUSPENDED;
      break;
      
    case M_COM:
      if (! (this->*cinfo->marker->process_COM) (cinfo))
	return JPEG_SUSPENDED;
      break;
...
// METHODDEF boolean
boolean SECJpeg::read_restart_marker (j_decompress_ptr cinfo)
{
  ...
  if ((cinfo->unread_marker ==
	  ...
	  // AAB: Needed to add this for canyon.jpg file...
	  || (cinfo->unread_marker == M_RST0)) {
    ...
    if (! (this->*cinfo->src->resync_to_restart) (cinfo,
...
// GLOBAL boolean
boolean SECJpeg::jpeg_resync_to_restart (j_decompress_ptr cinfo, int desired)
{
...
// METHODDEF void
void SECJpeg::reset_marker_reader (j_decompress_ptr cinfo)
{
...
// GLOBAL void
void SECJpeg::jinit_marker_reader (j_decompress_ptr cinfo)
{
  int i;
