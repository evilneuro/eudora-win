...
#define ADD_BASENAME(name) cdh_GLUE2(COMPONENT_BASENAME(),name)
	...
	typedef struct {
		short					rangeMax;
		cdhDispatchInfoRecord	*cdhDispatchInfoP;
	...
	#define ComponentRangeUnused(rangeNum) \
		static cdhDispatchInfoRecord cdh_GLUE2(cdhDispatchInfo,rangeNum)[1] = { 0 };	\
		enum {cdh_GLUE2(cdhDispatchMax,rangeNum) = 0};
		
	#define ComponentRangeBegin(rangeNum)	\
		static cdhDispatchInfoRecord cdh_GLUE2(cdhDispatchInfo,rangeNum)[] = {
	...
	static cdhRangeDispatchInfoRecord cdhRangeDispatch[RangeCount+1] = {
		#include COMPONENT_DISPATCH_FILE
	};
	...
	ComponentFunctionUPP COMPONENTSELECTORLOOKUP(short selector_num, ProcInfoType *procInfo)
	{
		...
		cdhDispatchInfoRecord *infoP = nil;
		short theRange;
	...
	ComponentResult OVERRIDE_CANDO(Handle storage, short ftnNumber, ComponentResult result);
#endif
