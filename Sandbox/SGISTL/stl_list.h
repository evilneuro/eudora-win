...
#  endif
    ...
    iterator erase_raw(iterator position) {
      __stl_debug_check(__check_if_owner(&iter_list,position));
      __stl_verbose_assert(position.node!=node, __STL_MSG_ERASE_PAST_THE_END);
...
#  endif
    ...
    iterator erase_raw(iterator first, iterator last);
...
template <class T, class Alloc>
__iterator__ 
list<T, Alloc>::erase_raw(iterator first, iterator last) {
  while (first != last) erase_raw(first++);
  return last;
}
